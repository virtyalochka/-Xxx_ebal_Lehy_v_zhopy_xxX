...
using namespace std;
    ...
    //Вывод одного узла
    void Tree::print1(Node* p) // p указатель на узел
    {
        cout << "     " << p->info << endl;
    }
    //Вывод всего дерева (поддерева) с узла p
    void Tree::print(Node* p, int tab)
    {
    ...
    Node* Tree::delete_listok(Node* listok) //Передаём указатель на узел листок для уничтожения
    {
    ...
    Node* Tree::Find_Node_by_info(Node* p, int info)//Программа находит по заданному значению узел дерева
    {
        if(p->info == info)
        {
            return p;
    ...
    void Tree::change_info(vector <int> v_new) //Нужно находить нужные элементы по старому значению, и менять в них значения на новые
    {
    ...
    vector <Node*> Tree::get_listiks(Node* p)
    {
        listiks.clear();
    ...
    vector <Node*> Tree::get_list_of_listiks(Node* p)
    {
        if(p->childs.size() == 0)
        {
    ...
    void Tree::print_listiks()// Печатает листки дерева
    {
        cout << "Листья дерева \n";
    ...
    Node* Tree::find_min_listiks()//Находит листик с минимальным значением
    {
    ...
    Node* Tree::delete_root_with_one_child()
    {
